...
export default function JukeboxPage() {
  ...
  const handleUploadSubmit = () => {
    ...
    createAudio.mutate({
      ...
      url: newTrack.url || "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3", // Demo URL
    }, {
    ...
    <div className="flex min-h-screen bg-background pb-24 md:pl-20 lg:pl-64">
      ...
      <div className="flex-1 p-6 md:p-8 max-w-7xl mx-auto w-full">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            <h1 className="text-3xl md:text-4xl font-display font-bold text-foreground">Jukebox</h1>
            <p className="text-muted-foreground mt-1">Discover and share music with the community</p>
          </div>
            ...
            <DialogContent className="bg-background/95 backdrop-blur-xl">
              ...
              <div className="space-y-4 py-4">
                ...
                <div className="border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center text-center gap-2 cursor-pointer hover:bg-muted/50 transition-colors">
                  ...
                  <p className="text-sm font-medium">Click to upload MP3 file</p>
                  <p className="text-xs text-muted-foreground">Max 10MB</p>
                </div>
                
                <Button className="w-full" onClick={handleUploadSubmit} disabled={createAudio.isPending}>
                ...
                >
                  <div className="aspect-square relative overflow-hidden bg-muted">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]">
                      ...
                      >
                        {isCurrent && isPlaying ? <Pause className="w-6 h-6" /> : <Play className="w-6 h-6 ml-1" />}
                      </Button>
                    </div>
                  </div>
