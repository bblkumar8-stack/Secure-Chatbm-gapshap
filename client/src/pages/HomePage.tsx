...
export default function HomePage() {
  ...
  const activeChatId = params.get("chatId") ? parseInt(params.get("chatId")!) : null;
  ...
  return (
    <div className="flex h-screen bg-background md:pl-20 lg:pl-64">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex justify-between items-center">
                      <p className="text-sm text-muted-foreground truncate pr-2">
                        {lastMsg ? (
                          <>
                            {lastMsg.senderId === user?.id && <span className="mr-1">You:</span>}
      ...
      {/* Main Chat Area */}
      <div className={`flex-1 bg-muted/30 relative flex flex-col ${!activeChatId ? "hidden md:flex" : "flex"}`}>
        {activeChatId ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground p-8 text-center animate-enter">
            ...
            <h2 className="text-2xl font-display font-bold text-foreground mb-2">Welcome to BmGapshap</h2>
            <p className="max-w-md">Select a chat from the sidebar or start a new conversation to get chatting. Don't forget to check out the Jukebox!</p>
          </div>
        )}
      </div>
    </div>
